...
export async function generateGeminiListeningQuestions(input:GenerateListeningQuestionsInput):Promise<GeneratedListeningQuestion[]>{
  ...
  for(let attempt=0;attempt<3;attempt+=1){try{
    ...
    if(input.part===1&&result.questions.some((question)=>JSON.stringify(question.image)!==JSON.stringify(input.image)))throw new Error("Part 1 圖片 metadata 不正確。");
...
function buildListeningPrompt(input:GenerateListeningQuestionsInput,attempt:number){
  ...
  return `${common}\nPart 1 Photographs：只能依這份圖片 metadata 出題：${JSON.stringify(input.image)}。每題 image 必須原樣回傳此 metadata；transcript 是依序朗讀 A/B/C/D 四個英文描述的文字；options 正好四個英文描述且只有一個符合圖片；question 填 Listen and choose the statement that best describes the photograph.；speakers 一位、dialogue=[]、audioGroupId 每題不同。禁止捏造 metadata 沒有的物件或動作。`;
}
