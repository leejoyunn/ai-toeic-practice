...
interface PublicQuestion { id:string;part:number;questionType:string;question:string;options:QuestionOption[];difficulty:Difficulty;targetScore:number;topic:string;scenario:string;grammarPoint:string;passage:string|null;passageType:string|null; }
...
export function ReadingPractice({part,signedIn}:{part:ReadingPart;signedIn:boolean}){
  const [stage,setStage]=useState<Stage>("setup");const [count,setCount]=useState(5);const [difficulty,setDifficulty]=useState<"auto"|Difficulty>("auto");
