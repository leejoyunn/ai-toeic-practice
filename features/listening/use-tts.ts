...
export function useTts(){
  const provider=useMemo(()=>new SpeechSynthesisProvider(),[]);const[status,setStatus]=useState<"idle"|"playing"|"paused">("idle");const[error,setError]=useState("");const[supported]=useState(()=>provider.isSupported());
