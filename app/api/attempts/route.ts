...
export async function POST(request:Request) {
  try {
    ...
    if(!isCorrect) await supabase.from("wrong_answers").upsert({user_id:user.id,question_id:question.id,first_attempt_id:attempt.id,wrong_count:1,last_wrong_at:new Date().toISOString(),next_review_at:new Date(Date.now()+86400000).toISOString()},{onConflict:"user_id,question_id"});
